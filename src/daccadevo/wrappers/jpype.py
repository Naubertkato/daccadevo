...
class Jpype_wrapper(DACCAD_Wrapper):
	...
	def generateAllConnections(self, array,inhibitions,graph, species_dict, nNodes = 5):
		...
		for fromNode in range(nNodes):
			for toNode in range(nNodes):
				...
				if array[index] > 0.0:
					edge = self.generateConnection(innovation, array[index], fromNode, toNode, graph, species_dict)
					edge_inhib_eq[f"I{fromNode}T{toNode}"]  = edge
					innovation += 1
		for val in inhibitions:
			concs, _ = inhibitions[val]

			for fromNode, concentration in concs:
				self.generateConnection(innovation, concentration, int(fromNode), val, graph, species_dict)
				graph.addInhibition(edge_inhib_eq[val], species_dict[val])
				innovation += 1
		return graph
	...
	# new
	def generateFull(self, array, graph = None, species_dict = {}, nNodes = 5,
		...
		if graph is None:
			graph = utils.GraphUtils.initGraph()
	...
	def submitPENSystem(self, array, nNodes = 5, config=None, **kwargs):
		...
		g = self.generateFull(array, nNodes =nNodes, **kwargs)
		# TODO: make
